...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('trainees.index') }}">{{ __('Trainers') }}</a></li>
...
@push('script-page')
    <script>
        ...
        function updateProgressBar() {
            ...
            $(".next").toggle(currentIndex !== totalTabs);
        }
        updateProgressBar();
            ...
            if (!element.checkValidity()) {
                ...
                if ($element.attr('name') === 'email') {
                    ...
                    if ($element.val().trim().length < 3) {
                        isValid = false;
                        errorMessage = 'Qualification must be at least 3 characters long';
                    }
                } else if ($element.attr('name') === 'assign_class[]') {
        ...
        <div class="col-sm-12">
            <div id="basicwizard" class="form-wizard row justify-content-center">
                <div class="col-12">
                    ...
                    {{ Form::model($trainer, ['route' => ['trainers.update', $trainer->id], 'method' => 'PUT', 'id' => 'businessForm','enctype' => 'multipart/form-data']) }}
                    <div class="card">
                        <div class="card-body">
                                            ...
                                            <div class="col">
                                                <div class="row">
                                                    ...
                                                    <div class="form-group col-md-6">
                                                        ...
                                                        {{ Form::date('dob', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->dob : null, ['class' => 'form-control', 'required' => 'required']) }}
                                                    </div>
                                                    <div class="form-group col-md-6">
                                                        {{ Form::label('gender', __('Gender'), ['class' => 'form-label']) }}
                                                        {!! Form::select('gender', $gender, !empty($trainer->trainerDetail) ? $trainer->trainerDetail->gender : null, [
                                                    ...
                                                    <div class="form-group col-md-6">
                                                        ...
                                                        {{ Form::text('qualification', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->qualification : null, ['class' => 'form-control', 'placeholder' => __('Enter qualification')]) }}
                                                    </div>
                                                    ...
                                                    <div class="form-group col-md-6 mb-3">
                                                        {{ Form::label('document', __('Document (Aadhar card, Pancard, Passport, Driving license)'), ['class' => 'form-label']) }}
                                    ...
                                    <div id='addressDetailForm'>
                                        <div class="row mt-4">
                                            <div class="form-group col-md-6">
                                                ...
                                                {{ Form::text('country', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->country : null, ['class' => 'form-control', 'placeholder' => __('Enter country')]) }}
                                            </div>
                                            <div class="form-group col-md-6">
                                                {{ Form::label('state', __('State'), ['class' => 'form-label']) }}
                                                {{ Form::text('state', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->state : null, ['class' => 'form-control', 'placeholder' => __('Enter state')]) }}
                                            </div>
                                            <div class="form-group col-md-6">
                                                {{ Form::label('city', __('City'), ['class' => 'form-label']) }}
                                                {{ Form::text('city', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->city : null, ['class' => 'form-control', 'placeholder' => __('Enter city')]) }}
                                            </div>
                                            <div class="form-group col-md-6">
                                                {{ Form::label('zip_code', __('Zip Code'), ['class' => 'form-label']) }}
                                                {{ Form::text('zip_code', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->zip_code : null, ['class' => 'form-control', 'placeholder' => __('Enter zip code')]) }}
                                            </div>

                                            <div class="form-group col-md-6">
                                                {{ Form::label('address', __('Address'), ['class' => 'form-label']) }}
                                                {{ Form::textarea('address', !empty($trainer->trainerDetail) ? $trainer->trainerDetail->address : null, ['class' => 'form-control', 'rows' => 2, 'placeholder' => __('Enter address')]) }}
                                            </div>
                                            <div class="form-group col-md-6">
                                                {{ Form::label('status', __('Status'), ['class' => 'form-label']) }}
                                                {!! Form::select('status', $status, !empty($trainer->trainerDetail) ? $trainer->trainerDetail->status : null, [
                                                    'class' => 'form-control select2',
                                                    'required' => 'required',
                                                ]) !!}
                                            </div>
